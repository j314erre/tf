...
# logger
logger = logging.getLogger("tensorflow_chat")
...
_BUCKETS = [(64, 32)]
#_BUCKETS = [(10, 10), (20, 15), (40, 25), (80, 50)]
        ...
        with gfile.GFile(target_path, mode="r") as target_file:
            ...
            while source_line and target_line and (not max_size or counter < max_size):
                ...
                for source_item in source_items[-context_window:]:                   
                    ...
                    source_ids_count += len(source_item_ids)
                ...
                for bucket_id, (encoder_size, decoder_size) in enumerate(buckets):
                    ...
                    if len(source_ids_list)+source_ids_count < encoder_size and len(target_ids) < decoder_size:
                        
                        encoder_inputs, decoder_inputs = pad_sequences(source_ids_list, encoder_size, target_ids, decoder_size)
...
def pad_sequences(source_sequence_list, encoder_size, target_sequence, decoder_size):
    ...
    # pack sequence list into one encoder list
    encoder_inputs = []
    for s, source_sequence in enumerate(source_sequence_list):
        
        # use EOS_ID between sequences
        if s>0:
            encoder_inputs.append(EOS_ID)
            
        # reverse each sequence
        for tok in reversed(source_sequence):
            encoder_inputs.append(tok)

        
    assert(len(encoder_inputs)<encoder_size)
    encoder_pad = [PAD_ID] * (encoder_size - len(encoder_inputs))
    
    # prepend padding 
    encoder_inputs = encoder_pad + encoder_inputs
...
def create_model(session, model_dir, source_vocab_size, target_vocab_size, buckets, size,
    ...
    model = TFSeq2SeqModel(source_vocab_size, target_vocab_size, buckets, size,
               ...
               num_samples, forward_only, summary=summary)
...
def train(input_file, model_dir, max_train_data_size, dev_train_split, steps_per_checkpoint, source_vocab_size, target_vocab_size, buckets, 
          max_context_window,
          size,
               num_layers, max_gradient_norm, batch_size, learning_rate,
               learning_rate_decay_factor, use_lstm=False,
    ...
    with tf.Session() as sess:
        ...
        model = create_model(sess, model_dir, source_vocab_size, target_vocab_size, buckets, size,
               num_layers, max_gradient_norm, batch_size, learning_rate,
               learning_rate_decay_factor, use_lstm,
               num_samples, forward_only, summary)
        ...
        while True:
            ...
            my_global_step, my_summary, _, step_loss, _ = model.step(sess, encoder_inputs, decoder_inputs,
    ...
    with tf.Session() as sess:
        ...
        for thread in threads:
            ...
            for sentence in lines:                
                ...
                source_ids_count += len(token_ids)
                sequences.append(token_ids)

            for bucket_id, (encoder_size, decoder_size) in enumerate(buckets):
                # pick the first bucket the data fits into
                # NOTE: if there is not bucket big enough data is not selected
                if len(sequences)+source_ids_count < encoder_size:
                    found_bucket = True
                    break
            if not found_bucket:
                continue
...
def main():
    parser = argparse.ArgumentParser(description='Chatbot: Attention Encoder-Decoder')
