...
from tensorflow_Nseq2seq_model_b import TFNSeq2SeqModel
from tensorflow.python.platform import gfile

# logger
logger = logging.getLogger("tensorflow_chat2b")
...
def get_batch(data, encoder_size, decoder_size, context_window, batch_size, randomize=True):
    ...
    for idx in xrange(batch_size):
        ...
        assert len(encoder_input_list)==context_window, "encoder_input_list=%s" % encoder_input_list
...
def train(input_file, model_dir, max_train_data_size, dev_train_split, 
    ...
    with tf.Session() as sess:
        ...
        while True:
            ...
            _, step_loss, _ = model.step(sess, encoder_inputs_matrix, decoder_inputs,
                    ...
                    for bucket_id in xrange(len(buckets)):
                        ...
                        if dev_set_bucket:
                            ...
                            while dev_set_batch_offset < len(dev_set_bucket):
                                ...
                                _, eval_loss, _ = model.step(session=sess, 
...
def decode(threads, model_dir, source_vocab_size, target_vocab_size, buckets, 
    ...
    with tf.Session() as sess:
        ...
        for thread in threads:
            ...
            _, _, output_logits = model.step(session=sess, 
                                             encoder_inputs_matrix=encoder_inputs_matrix, 
                                             decoder_inputs=decoder_inputs,
                                             target_weights=target_weights, 
                                             bucket_id=bucket_id, 
...
def self_test():
    ...
    fake_data = [
                 ...
                 ([[3, 4, 5, 6, 1], [4, 5, 6, 1, 2]], [5, 6, 1, 2, 3]), 
                 ([[4, 5, 6, 1, 2], [5, 6, 1, 2, 3]], [6, 1, 2, 3, 4]), 
                 ([[5, 6, 1, 2, 3], [1, 2, 3, 4, 5]], [1, 3, 4, 5, 6]), 
                 ([[1, 2, 3, 4, 5], [2, 3, 4, 5, 6]], [3, 4, 5, 6, 1]), 
                 ([[2, 3, 4, 5, 6], [3, 4, 5, 6, 1]], [4, 5, 6, 1, 2]), 
                 ([[3, 4, 5, 6, 1], [4, 5, 6, 1, 2]], [5, 6, 1, 2, 3]), 
                 ([[4, 5, 6, 1, 2], [5, 6, 1, 2, 3]], [6, 1, 2, 3, 4]), 
    ...
    with tf.Session(config=tf.ConfigProto(log_device_placement=True)) as sess:
        ...
        for _ in xrange(5):  # Train the fake model for 5 steps.
            ...
            encoder_inputs_matrix, decoder_inputs, target_weights = get_batch(data=data_set[bucket_id], 
                                                                       ...
                                                                       batch_size=batch_size, 
                                                                       randomize=True)
            _, step_loss, _ = model.step(sess, encoder_inputs_matrix, decoder_inputs, target_weights,
                     bucket_id, False)
            loss += step_loss
