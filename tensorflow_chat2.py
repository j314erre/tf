...
from tensorflow_Nseq2seq_model import TFNSeq2SeqModel
from tensorflow.python.platform import gfile

# logger
logger = logging.getLogger("tensorflow_chat2")
    ...
    with gfile.GFile(source_path, mode="r") as source_file:
        with gfile.GFile(target_path, mode="r") as target_file:
            ...
            while source_line and target_line and (not max_size or counter < max_size):
                ...
                target_ids = [int(x) for x in target_line.strip().split()]
                ...
                for bucket_id, (encoder_size, decoder_size) in enumerate(buckets):
                    ...
                    if all(len(source_ids) < encoder_size for source_ids in source_ids_list) and len(target_ids) < decoder_size:
                        ...
                        data_set[bucket_id].append([encoder_inputs_list, decoder_inputs])
                        break
...
def get_batch(data, encoder_size, decoder_size, context_window, batch_size, randomize=True):
    ...
    encoder_inputs_matrix, decoder_inputs = [], []
    ...
    for idx in xrange(batch_size):
        ...
        if randomize is True:
            encoder_input_list, decoder_input = random.choice(data)
        else:
            encoder_input_list, decoder_input = data[idx]
            
        # make sure data matches the model
        assert len(encoder_input_list)==context_window
    ...
    batch_encoder_inputs_matrix, batch_decoder_inputs, batch_weights = [], [], []
    ...
    return batch_encoder_inputs_matrix, batch_decoder_inputs, batch_weights
...
def train(input_file, model_dir, max_train_data_size, dev_train_split, 
          ...
          size,
               num_layers, max_gradient_norm, batch_size, learning_rate,
               learning_rate_decay_factor, use_lstm=False,
               num_samples=512, forward_only=False, checkpoints=0, max_global_step=0, summary=False):
    ...
    with tf.Session() as sess:
        ...
        while True:
            ...
            encoder_inputs_matrix, decoder_inputs, target_weights  = get_batch(data=train_set[bucket_id], 
            ...
            my_global_step, my_summary, _, step_loss, _ = model.step(sess, encoder_inputs_matrix, decoder_inputs,
            ...
            if current_step % steps_per_checkpoint == 0:
                ...
                logger.info("Checkpoint: global_step: %d learning_rate: %f step_time: %.2f loss: %f, perplexity: %f tally: %s" 
                            % (my_global_step, model.learning_rate.eval(),
                                 avg_step_time, loss, perplexity, bucket_tally))
                ...
                if dev_set:
                    ...
                    for bucket_id in xrange(len(buckets)):
                        ...
                        if dev_set_bucket:
                            ...
                            while dev_set_batch_offset < len(dev_set_bucket):
                                ...
                                encoder_inputs_matrix, decoder_inputs, target_weights = get_batch(data=dev_set_batch, 
                                                                                           encoder_size=buckets[bucket_id][0], 
                                                                                           decoder_size=buckets[bucket_id][1],
                                                                                           context_window=max_context_window,
                                ...
                                logger.info("dev_step: bucket: %d step: %d batch_size: %d step_loss: %f step_time: %.2f " 
                                            % (bucket_id, dev_set_batch_count, model.batch_size, eval_loss, dev_set_batch_time))
                    ...
                    logger.info("Evaluate: global_step: %d dev_size: %d dev_time: %.2f dev_loss: %s dev_perplexity: %s" % (my_global_step, dev_total, dev_time, dev_loss, dev_perplexity))
...
def decode(threads, model_dir, source_vocab_size, target_vocab_size, buckets, 
    ...
    with tf.Session() as sess:
        ...
        for thread in threads:
            ...
            data_set = [(padded_token_ids_list, padded_decoder_ids)]

            # Get a 1-element batch to feed the sentence to the model.
            encoder_inputs_matrix, decoder_inputs, target_weights = get_batch(
                                                                       data=data_set,
                                                                       encoder_size=encoder_size, 
                                                                       decoder_size=decoder_size,
                                                                       context_window=max_context_window,
            ...
            _, _, _, _, output_logits = model.step(session=sess, 
                                             encoder_inputs_matrix=encoder_inputs_matrix, 
                                             decoder_inputs=decoder_inputs,
                                             target_weights=target_weights, 
                                             bucket_id=bucket_id, 
            ...
            logger.info("DECODE input=[%s] output=[%s]" % ("|".join(lines), " ".join([rev_target_vocab[output] for output in outputs])))
...
def self_test():
    ...
    # Fake data set...
    fake_data = [
                 ([[4, 5, 6, 7, 8], [5, 6, 7, 8, 9]], [6, 7, 8, 9, 4]), 
                 ([[5, 6, 7, 8, 9], [6, 7, 8, 9, 4]], [7, 8, 9, 4, 5]), 
                 ([[6, 7, 8, 9, 4], [7, 8, 9, 4, 5]], [8, 9, 4, 5, 6]), 
                 ([[7, 8, 9, 4, 5], [8, 9, 4, 5, 6]], [9, 4, 5, 6, 7]), 
                 ([[8, 9, 4, 5, 6], [9, 4, 5, 6, 7]], [4, 5, 6, 7, 8]), 
                 ([[4, 5, 6, 7, 8], [5, 6, 7, 8, 9]], [6, 7, 8, 9, 4]), 
                 ([[5, 6, 7, 8, 9], [6, 7, 8, 9, 4]], [7, 8, 9, 4, 5]), 
                 ([[6, 7, 8, 9, 4], [7, 8, 9, 4, 5]], [8, 9, 4, 5, 6]), 
                 ([[7, 8, 9, 4, 5], [8, 9, 4, 5, 6]], [9, 4, 5, 6, 7]), 
    ...
    data_set = [[] for _ in buckets]
    for (source_ids_list, target_ids) in fake_data:
        for bucket_id, (encoder_size, decoder_size) in enumerate(buckets):
            # pick the first bucket the data fits into
            # NOTE: if there is not bucket big enough data is not selected
            if all(len(source_ids) < encoder_size for source_ids in source_ids_list) and len(target_ids) < decoder_size:
    ...
    with tf.Session(config=tf.ConfigProto(log_device_placement=True)) as sess:
        logger.info("Self-test for Nseq2seq model.")
        ...
        # 2 layers of size 32
        model = TFNSeq2SeqModel(source_vocab_size=10, 
        ...
        for _ in xrange(5):  # Train the fake model for 5 steps.
            ...
            encoder_inputs_matrix, decoder_inputs, target_weights = get_batch(data=data_set[bucket_id], 
                                                                       encoder_size=buckets[bucket_id][0], 
                                                                       decoder_size=buckets[bucket_id][1],
                                                                       context_window=context_window,
                                                                       batch_size=batch_size, 
                                                                       randomize=True)
            _, _, _, step_loss, _ = model.step(sess, encoder_inputs_matrix, decoder_inputs, target_weights,
                     bucket_id, False)
            loss += step_loss
...
def main():
    parser = argparse.ArgumentParser(description='Chatbot2: Nseq2seq')
